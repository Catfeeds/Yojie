...
$('.formBar button[type=submit]',$p).on('click',function(){
    $("input.js-getLnglat",$p).change();
})
$("input.js-getLnglat",$p).on("change",function() {
	var $this=$(this);
	var address= $this.val();
	var inputs=$this.parent().parent().siblings('.display_none').find('input');
	console.log(inputs);
	console.log(address)
        ...
        new Promise((resolve,reject)=>{
            ...
            geocoder.getLocation(address, function(status, result) {
                ...
                if (status === 'complete'&&result.geocodes.length) {
					...
					console.log(lnglat.O);
					if(lnglat==undefined){
                        // alert('地址不够详细，请修改后在试');
					}else{
                        inputs[4]=lnglat.O;
                        inputs[5]=lnglat.N;
                        // alert(lnglat.O+','+lnglat.N);
					}
