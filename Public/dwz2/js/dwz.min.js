...
//地址组件
    ...
    loadMap().then(map=>loadMapPlugin(map,plugins));
    $("a.js-inputCheckMap",$p).on("click",function(){
		var $target={};
        $target['s_address']=$(this).parent().find('input[name="areainfo[areainfomap4][address]"]');
        $target['s_coordinatex']=$(this).parent().find('input[name="areainfo[areainfomap4][coordinatex]"]');
        $target['s_coordinatey']=$(this).parent().find('input[name="areainfo[areainfomap4][coordinatey]"]');
        var $inputsParent = $(this).parent().next();
        var inputsName=['province','city','area','town','street','longitude','latitude','formattedAddress'];
        var $inputs=inputsName.map(item=>`input[name="areainfo[areainfomap4][${item}]"]`).map(item=>$inputsParent.find(item));

		for(let i=0;i<inputsName.length;i++){
            $target[inputsName[i]]=$inputs[i];
		}
        function mapInit(AMap,mapId,inputId){
            ...
            //点击地图选址
            map.on('click',function(e){regeoCode(e.lnglat,map)});
            //搜素自动完成
            ...
            AMap.event.addListener(placeSearch, "markerClick", function(e){
                regeoCode(e.data.location,map);
            });
            //数据带回
            lookup_address(map,$target);
            // 根据纬度在地图上标注位置和返回地理编码地址
            function regeoCode(lnglat,map) {
                return new Promise((resolve,reject)=>{
                    if(!geocoder){
                    ...
                    geocoder.getAddress(lnglat, function(status, result) {
                        if (status === 'complete'&&result.regeocode) {
                            ...
                            // var detail=address.street+address.streetNumber;
                            //地图组件map承载结果
							map._result={province,city,district,town,street,formattedAddress:result.regeocode.formattedAddress,lng:lnglat.N,lat:lnglat.O};
            ...
            function lookup_address(map,$target){
                ...
                sure.addEventListener('click', function(){
                    ...
                    // $target['s_coordinatex'].val(result.lng);
                    // $target['s_coordinatey'].val(result.lat);
                    $target['province'].val(result.province);
                    $target['city'].val(result.city);
                    $target['area'].val(result.district);
                    $target['town'].val(result.town);
                    // $target['street'].val(result.street);

                    // $target['longitude'].val(result.lng);
                    // $target['latitude'].val(result.lat);
                    $target['formattedAddress'].val(result.formattedAddress);

                    var addressObj=null;
                    if (['北京市', '天津市', '上海市', '重庆市'].indexOf(map._result.province) > 0) {
                    ...
                    } else if (map._result.province == '海南省') {
                        if (['海口市', '三亚市', '三沙市'].indexOf(map._result.city) > 0) {
                            addressObj = {
                                'level1': result.province,
                                'level2': result.city,
                                'level3': result.district,
                    ...
                    var clevel1= $("select.nbmxkjlevel1",$p);
        ...
        var plugins=['AMap.Geolocation','AMap.Autocomplete','AMap.PlaceSearch','AMap.Geocoder','AMap.Scale','AMap.ToolBar'];
        if(window.amaps){
            mapInit(window.amaps,'gddt','autoInput')
        }else{
...
//有地址组件的时候给提交按钮注册查询经纬度事件
$('.address_elm',$p).parent().parent().find('.formBar button[type="submit"]').on('click',function(event){
	var that=$(this)
    var $input_address=$("input.js-getLnglat",$p);
	if($input_address.length>0){
        var address=$input_address.val();
        var inputs=$input_address.parent().parent().siblings('.display_none');
        var longitude=inputs.find('input[name="areainfo[areainfomap4][longitude]"]');
        var latitude=inputs.find('input[name="areainfo[areainfomap4][latitude]"]');
        var flag=!!that.attr('flag');
        if(!flag){
            event.preventDefault();
            if(!address.trim()){
                alertMsg.error('地址为空，选择地址后重试');
            }else{
                geoCode(address,function(lnglat){
                    if(!flag){
...
if($.fn.cascade){
		...
		$('select[cascade]').cascade.afterchange=function(curobj , box){
			...
			var inputObj={privince:$("input[name='areainfo[areainfomap4][province]']",hidden_box),
