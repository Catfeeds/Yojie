        ...
        function mapInit(AMap,mapId,inputId){
                ...
                return new Promise((resolve,reject)=>{
                    ...
                    geocoder.getAddress(lnglat, function(status, result) {
                        if (status === 'complete'&&result.regeocode) {
							...
							map._result={province,city,district,street,formattedAddress:result.regeocode.formattedAddress,lng:lnglat.N,lat:lnglat.O};
            ...
            function registerEvent(){
                ...
                sure.addEventListener('click', function(){
                    ...
                    $prev.value=map._result.province+map._result.city+map._result.district+map._result.street;

                    $next[0].value=map._result.province;
                    $next[1].value=map._result.city;
                    $next[2].value=map._result.district;
                    $next[3].value=map._result.street;
                    $next[4].value=map._result.lng;
                    $next[5].value=map._result.lat;
                    $next[6].value=map._result.formattedAddress;
                    console.log(map._result.city);
                    var addressObj=null
                    ...
                    if (['北京市', '天津市', '上海市', '重庆市'].indexOf(map._result.province) > 0) {
                        ...
                        addressObj = {
                            'level1': map._result.province,
                            'level2': level2,
                            'level3': map._result.district,
                            'level4': map._result.street,
                            'map': true
                        }
                    ...
                    $('.address_four_level',$p).val('');
                    clevel1.val(clevel1.find(`option:contains(${map._result.province})`).val()).attr('address',addressStr).change();
                });
            }
        }
...
$('.formBar button[type=submit]',$p).on('click',function(){
    ...
    var inputs=$input_address.parent().parent().siblings('.display_none').find('input');
    if(!address.trim()){
        ...
        new Promise((resolve,reject)=>{
            ...
            geocoder.getLocation(address, function(status, result) {
                if (status === 'complete'&&result.geocodes.length) {
                    ...
                    console.log(lnglat.O,lnglat.N);
                    if(lnglat==null){
                        alertMsg.error('地址不够详细，请修改后在试');
                    }else{
                        inputs[4]=lnglat.O;
                        inputs[5]=lnglat.N;
                    }
