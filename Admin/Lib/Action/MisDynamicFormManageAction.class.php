...
// 			$MisDynamicFormTemplateModel->commit();
	...
	public function looktablename($type,$name){
		...
		//根据表名称查询是否重复
		$sql="SELECT	TABLES.TABLE_NAME FROM information_schema.TABLES WHERE TABLES.TABLE_SCHEMA = '".C('DB_NAME')."' and TABLES.TABLE_NAME='".$tablename."'";
		$resultList=$MisDynamicFormManageModel->query($sql);
		$result=$MisDynamicFormManageModel->query($sql);
		if($result){
