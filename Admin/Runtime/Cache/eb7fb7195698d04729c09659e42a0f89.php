<?php if (!defined('THINK_PATH')) exit();?><script src="__TMPL__MisAutoAhf/edit.js" type="text/javascript"></script><script src="__TMPL__MisAutoAhf/editExtend.js" type="text/javascript"></script><script src="__PUBLIC__/Js/layer/layer.js" type="text/javascript"></script><script type="text/javascript">
    //多个细分品类值时带回保存处理
...
<?php $classNodeSettingArr =getModelClassByNodeSetting('MisAutoAhf','edit');?>
<?php $appendPageContentArr =getBindTabsContent('MisAutoAhf',$vo,'edit','',$main);?>
<?php $formautosetting = setFormControllAutoCreteAppend('MisAutoAhf' ,'edit' ,$main , 'update',$vo,A('MisAutoAhf')->rebuildSetting());?>
...
<div class="page" <?php if(MisAutoAhf == 'MisAutoIbf'): ?>onmouseover="xifenvalch()"<?php endif; ?>>
	<div class="pageContent">
		<div class="pageFormContent applecloth anchorsToolBarParen"
			<?php if(!$_REQUEST['main'] or $_REQUEST['main'] == MODULE_NAME): if(!$_REQUEST['bindaname']): ?>layoutH="40"<?php endif; endif; ?>
			>
			<div class="new_version_page ">
				<form id="MisAutoAhf_edit"
					<?php echo ($appendPageContentArr[5]); ?> <?php echo ($formautosetting[3]); ?>  method="post" action="__APP__/MisAutoAhf/<?php echo ($formautosetting[2]); ?>/navTabId/__MODULE__<?php echo ($formautosetting[4]); ?>" class="pageForm required-validate"	 
					onsubmit="<?php if($formautosetting[5]): echo ($formautosetting[5]); else: echo ($appendPageContentArr[0]); endif; ?>">
					<a class='xyz_anchornavi_top' name='MisAutoAhf_edit_top'></a> 
					<input type="hidden" name="callbackType" value="closeCurrent" />
					<?php echo W('HiddenInput',$vo);?> 
					<input type="hidden" name="id" value="<?php echo ($vo['id']); ?>" />
					<input type="hidden" name="masid" value="<?php echo ($vo['id']); ?>" />
					<input type="hidden" name="shujukumingcheng" value="<?php echo ($shujukumingcheng); ?>" />
					...
					<?php if($_GET['viewtype'] != 'view'&& !$_GET['main'] ): ?><div class="new_version_page_header pageFormContent ">
						...
						<?php echo W('ShowRightToolBar',array('edit','MisAutoAhf',$vo));?>
						<?php echo W('ShowAnchorNavi',array('MisAutoAhf', 'edit',$main));?>
					...
					<div class="new_version_page_content">
                        ...
                        <div class="col_1_7 form_group_lay field_areainfomap4" original="<?php echo ($vo['areainfomap4']); ?>" category="areainfomap" style="">
                            ...
                            <div class="display_none <?php echo ($classNodeSettingArr['areainfomap4']); ?>">
                                <input type="hidden" name="areainfoMap[areainfomap4][province]" value="<?php echo ($areainfoMap['areainfomap4']['province']); ?>">
                                <input type="hidden" name="areainfoMap[areainfomap4][city]" value="<?php echo ($areainfoMap['areainfomap4']['city']); ?>">
                                <input type="hidden" name="areainfoMap[areainfomap4][area]" value="<?php echo ($areainfoMap['areainfomap4']['area']); ?>">
                                <input type="hidden" name="areainfoMap[areainfomap4][address]" value="<?php echo ($areainfoMap['areainfomap4']['address']); ?>">
                                <input type="hidden" name="areainfoMap[areainfomap4][lng]" value="<?php echo ($areainfoMap['areainfomap4']['lng']); ?>"/>
                                <input type="hidden" name="areainfoMap[areainfomap4][lat]" value="<?php echo ($areainfoMap['areainfomap4']['lat']); ?>"/>
                                <input type="hidden" name="areainfoMap[areainfomap4][formattedAddress]" value="<?php echo ($areainfoMap['areainfomap4']['detail']); ?>"/>
                            </div>
                        </div>
